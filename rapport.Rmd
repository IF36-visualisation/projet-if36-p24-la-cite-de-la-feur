...
Sur ce graphique, les points qui se détachent de la base de points noirs sont
...
Attention, encore une fois à l'échelle de ce graphique : comme nous avons une
très forte disparité entre le nombre d'étoile des dépôts, nous affichons en même temps
des dépôts avec un nombre d'étoile en centaine ($10^2$) et des dépôts avec 
...
```{r}
ggplot(df[df$stars >= 5000,], aes(x = creationDate, y = stars)) +
    geom_point(size = 0.5) +
    scale_x_date(breaks = datebreaks) +
    # pour afficher les dates en biais
    theme(axis.text.x = element_text(angle = 30, hjust = 1)) +
